...
PPMImage* changeImage(PPMImage* im)
{
	...
	*/
	...
	{	
		double temp_red_mean = 0.0;
		double temp_grn_mean = 0.0;
		double temp_blu_mean = 0.0;
		#pragma omp for
		for(x = 0; x < rows; x++)
		{
			for(y = 0; y < cols; y++) 
			{
				int idx = (x * cols) + y;
				temp_red_mean += ((double) (im->data + idx)->red);
				temp_grn_mean += ((double) (im->data + idx)->green);
				temp_blu_mean += ((double) (im->data + idx)->blue);
			}
		}
		#pragma omp critical 
		{
			red_mean += temp_red_mean;
			grn_mean += temp_grn_mean;
			blu_mean += temp_blu_mean;
		}
	}
	red_mean /= (rows * cols);
	grn_mean /= (rows * cols);
	...
	#pragma omp parallel private(x, y)
	{
		double private_red_std_dev = 0.0;
		double private_grn_std_dev = 0.0;
		double private_blu_std_dev = 0.0;				
		#pragma omp for
		for(x = 0; x < rows; x++)
		{
			for(y = 0; y < cols; y++) 
			{
				int idx = (x * cols) + y;
				private_red_std_dev += ((((double) (im->data + idx)->red) - red_mean) * (((double) (im->data + idx)->red) - red_mean));
				private_grn_std_dev += ((((double) (im->data + idx)->green) - grn_mean) * (((double) (im->data + idx)->green) - grn_mean));
				private_blu_std_dev += ((((double) (im->data + idx)->blue) - blu_mean) * (((double) (im->data + idx)->blue) - blu_mean));
			}
		}
		
		#pragma omp critical
		{
			red_std_dev += private_red_std_dev;
			grn_std_dev += private_grn_std_dev;
			blu_std_dev += private_blu_std_dev;
		}
	}
...
int main(int argc, char* argv[])
{
	...
	strcat(outputImageName,"_normalization_parallel.ppm");
